...
class FDIPTLB(instruction: Boolean, lgMaxSize: Int, cfg: TLBConfig)(implicit edge: TLEdgeOut, p: Parameters) extends CoreModule()(p) {
  override def desiredName = if (instruction) "ITLB" else "DTLB"
  val io = IO(new Bundle {
    //目前只有端口0才会发出PTW,d端口1只做旁路查询
    /** request from Core */
    val req = Vec(2,Flipped(Decoupled(new TLBReq(lgMaxSize))))
    /** response to Core */
    val resp = Output(Vec(2,new TLBResp(lgMaxSize)))
    /** SFence Input */
  ...
  val vpn = io.req(0).bits.vaddr(vaddrBits-1, pgIdxBits)
  val pref_vpn = io.req(1).bits.vaddr(vaddrBits-1, pgIdxBits)
  /** index for sectored_Entry */
  val memIdx = vpn.extract(cfg.nSectors.log2 + cfg.nSets.log2 - 1, cfg.nSectors.log2)
  val pref_memIdx = pref_vpn.extract(cfg.nSectors.log2 + cfg.nSets.log2 - 1, cfg.nSectors.log2)
  /** TLB Entry */
  ...
  def pref_ordinary_entries = sectored_entries(memIdx) ++ superpage_entries
  def pref_all_entries      = pref_ordinary_entries ++ special_entry
  ...
  /** privilege mode */
  val priv = io.req(0).bits.prv
  val priv_v = usingHypervisor.B && io.req(0).bits.v
  val priv_s = priv(0)
  val pref_priv = io.req(1).bits.prv
  val pref_priv_v = usingHypervisor.B && io.req(1).bits.v
  val pref_priv_s = pref_priv(0)
  ...
  val pref_vstage1_en = usingHypervisor.B && pref_priv_v && io.ptw.vsatp.mode(io.ptw.vsatp.mode.getWidth-1)
  ...
  /** Enable Virtual Memory when:
    ...
    */
  val vm_enabled = (stage1_en || stage2_en) && priv_uses_vm && !io.req(0).bits.passthrough

  // flush guest entries on vsatp.MODE Bare <-> SvXX transitions
  val v_entries_use_stage1 = RegInit(false.B)
  val vsatp_mode_mismatch  = priv_v && (vstage1_en =/= v_entries_use_stage1) && !io.req(0).bits.passthrough
  val pref_vsatp_mode_mismatch = pref_priv_v && (pref_vstage1_en =/= v_entries_use_stage1) && !io.req(1).bits.passthrough
  ...
  val mpu_ppn = Mux(do_refill, refill_ppn,
                Mux(vm_enabled && special_entry.nonEmpty.B, special_entry.map(e => e.ppn(vpn, e.getData(vpn))).getOrElse(0.U), io.req(0).bits.vaddr >> pgIdxBits))
  val mpu_physaddr = Cat(mpu_ppn, io.req(0).bits.vaddr(pgIdxBits-1, 0))
  val mpu_priv = Mux[UInt](usingVM.B && (do_refill || io.req(0).bits.passthrough /* PTW */), PRV.S.U, Cat(io.ptw.status.debug, priv))
  val pmp = Module(new PMPChecker(lgMaxSize))
  pmp.io.addr := mpu_physaddr
  pmp.io.size := io.req(0).bits.size
  ...
  val pref_hitsVec = pref_all_entries.map(vm_enabled && _.hit(pref_vpn, pref_priv_v))
  val real_hits = hitsVec.asUInt
  val hits = Cat(!vm_enabled, real_hits)
  val pref_hits = Cat(!vm_enabled, pref_hitsVec.asUInt)
  ...
  val pref_entries = pref_all_entries.map(_.getData(pref_vpn))

  val normal_entries = entries.take(ordinary_entries.size)
  val pref_normal_entries = pref_entries.take(pref_ordinary_entries.size)
  ...
  val pref_ppn = Mux1H(pref_hitsVec :+ !vm_enabled, (pref_all_entries zip pref_entries).map{ case (entry, data) => entry.ppn(pref_vpn, data) } :+ pref_vpn(ppnBits-1, 0))
  ...
  val ptw_ae_array_pref = Cat(false.B, pref_entries.map(_.ae_ptw).asUInt)
  val final_ae_array_pref = Cat(false.B, pref_entries.map(_.ae_final).asUInt)
  val ptw_pf_array_pref = Cat(false.B, pref_entries.map(_.pf).asUInt)
  val ptw_gf_array_pref = Cat(false.B, pref_entries.map(_.gf).asUInt)
  // privilege check
  val priv_x_ok_pref = Mux(pref_priv_s, ~pref_entries.map(_.u).asUInt, pref_entries.map(_.u).asUInt)
  val stage1_bypass_pref = Fill(pref_entries.size, usingHypervisor.B) & (Fill(pref_entries.size, !stage1_en) | pref_entries.map(_.ae_stage2).asUInt)
  val x_array_pref = Cat(true.B, (priv_x_ok_pref & pref_entries.map(_.sx).asUInt) | stage1_bypass_pref)
  val px_array_pref = Cat(Fill(nPhysicalEntries, prot_x), pref_normal_entries.map(_.px).asUInt) & ~(ptw_ae_array_pref | final_ae_array_pref)
  
  val pref_pf_fault = ((~x_array_pref & ~ptw_ae_array_pref) | ptw_pf_array_pref) & ~ptw_gf_array_pref
  ...
  // val pref_pr_array = Cat(Fill(nPhysicalEntries, prot_r), pref_ordinary_entries.map(_.pr).asUInt) & ~(ptw_ae_array | final_ae_array)
  ...
  val misaligned = (io.req(0).bits.vaddr & (UIntToOH(io.req(0).bits.size) - 1.U)).orR
  def badVA(guestPA: Boolean): Bool = {
    ...
    (for (i <- 0 until nPgLevelChoices) yield {
      ...
      val maskedVAddr = io.req(0).bits.vaddr & mask
      additionalPgLevels === i.U && !(maskedVAddr === 0.U || signed.B && maskedVAddr === mask)
    }).orR
  }
  def pref_badVA(guestPA: Boolean): Bool = {
    val additionalPgLevels = (if (guestPA) io.ptw.hgatp else satp).additionalPgLevels
    val extraBits = if (guestPA) hypervisorExtraAddrBits else 0
    val signed = !guestPA
    val nPgLevelChoices = pgLevels - minPgLevels + 1
    ...
    (for (i <- 0 until nPgLevelChoices) yield {
      val mask = ((BigInt(1) << vaddrBitsExtended) - (BigInt(1) << (minVAddrBits + i * pgLevelBits - signed.toInt))).U
      val maskedVAddr = io.req(1).bits.vaddr & mask
      additionalPgLevels === i.U && !(maskedVAddr === 0.U || signed.B && maskedVAddr === mask)
    }).orR
  }
  val bad_gpa =
  ...
  val pref_bad_va =
    if (!usingVM || (minPgLevels == pgLevels && vaddrBits == vaddrBitsExtended)) false.B
    else vm_enabled && stage1_en && pref_badVA(false)
  
  val cmd_lrsc = usingAtomics.B && io.req(0).bits.cmd.isOneOf(M_XLR, M_XSC)
  val cmd_amo_logical = usingAtomics.B && isAMOLogical(io.req(0).bits.cmd)
  val cmd_amo_arithmetic = usingAtomics.B && isAMOArithmetic(io.req(0).bits.cmd)
  val cmd_put_partial = io.req(0).bits.cmd === M_PWR
  val cmd_read = isRead(io.req(0).bits.cmd)
  val cmd_readx = usingHypervisor.B && io.req(0).bits.cmd === M_HLVX
  val cmd_write = isWrite(io.req(0).bits.cmd)
  val cmd_write_perms = cmd_write ||
    io.req(0).bits.cmd.isOneOf(M_FLUSH_ALL, M_WOK) // not a write, but needs write permissions
  ...
  val pref_tlb_miss = vm_enabled && !vsatp_mode_mismatch && !pref_bad_va && !(pref_hitsVec.asUInt & gpa_hits).orR
  ...
  when (io.req(0).valid && vm_enabled) {
    // replace
  ...
  io.req(0).ready := state === s_ready
  // page fault
  io.resp(0).pf.ld := (bad_va && cmd_read) || (pf_ld_array & hits).orR
  io.resp(0).pf.st := (bad_va && cmd_write_perms) || (pf_st_array & hits).orR
  io.resp(0).pf.inst := bad_va || (pf_inst_array & hits).orR
  // guest page fault
  io.resp(0).gf.ld := (bad_gpa && cmd_read) || (gf_ld_array & hits).orR
  io.resp(0).gf.st := (bad_gpa && cmd_write_perms) || (gf_st_array & hits).orR
  io.resp(0).gf.inst := bad_gpa || (gf_inst_array & hits).orR
  // access exception
  io.resp(0).ae.ld := (ae_ld_array & hits).orR
  io.resp(0).ae.st := (ae_st_array & hits).orR
  io.resp(0).ae.inst := (~px_array & hits).orR
  // misaligned
  io.resp(0).ma.ld := misaligned && cmd_read
  io.resp(0).ma.st := misaligned && cmd_write
  io.resp(0).ma.inst := false.B // this is up to the pipeline to figure out
  io.resp(0).cacheable := (c_array & hits).orR
  io.resp(0).must_alloc := (must_alloc_array & hits).orR
  io.resp(0).prefetchable := (prefetchable_array & hits).orR && edge.manager.managers.forall(m => !m.supportsAcquireB || m.supportsHint).B
  io.resp(0).miss := do_refill || vsatp_mode_mismatch || tlb_miss || multipleHits
  io.resp(0).paddr := Cat(ppn, io.req(0).bits.vaddr(pgIdxBits-1, 0))
  io.resp(0).size := io.req(0).bits.size
  io.resp(0).cmd := io.req(0).bits.cmd
  io.resp(0).gpa_is_pte := vstage1_en && r_gpa_is_pte
  io.resp(0).gpa := {
    val page = Mux(!vstage1_en, Cat(bad_gpa, vpn), r_gpa >> pgIdxBits)
    val offset = Mux(io.resp(0).gpa_is_pte, r_gpa(pgIdxBits-1, 0), io.req(0).bits.vaddr(pgIdxBits-1, 0))
    Cat(page, offset)
  }
  io.req(1).ready := state === s_ready
  // page fault
  io.resp(1).pf.ld := false.B
  io.resp(1).pf.st := false.B
  io.resp(1).pf.inst := pref_bad_va || (pref_pf_fault & pref_hits).orR
  // guest page fault
  io.resp(1).gf.ld := false.B
  io.resp(1).gf.st := false.B
  io.resp(1).gf.inst := bad_gpa // we dont use it
  // access exception
  io.resp(1).ae.ld := false.B
  io.resp(1).ae.st := false.B
  io.resp(1).ae.inst := (~px_array_pref & pref_hits).orR
  // misaligned
  io.resp(1).ma.ld        := misaligned && cmd_read
  io.resp(1).ma.st        := misaligned && cmd_write
  io.resp(1).ma.inst      := false.B // this is up to the pipeline to figure out
  io.resp(1).cacheable    := (c_array & hits).orR
  io.resp(1).must_alloc   := (must_alloc_array & hits).orR
  io.resp(1).prefetchable := (prefetchable_array & hits).orR && edge.manager.managers.forall(m => !m.supportsAcquireB || m.supportsHint).B
  io.resp(1).miss         := do_refill || vsatp_mode_mismatch || tlb_miss || multipleHits
  io.resp(1).paddr        := Cat(pref_ppn, io.req(1).bits.vaddr(pgIdxBits-1, 0))
  io.resp(1).size         := io.req(1).bits.size
  io.resp(1).cmd          := io.req(1).bits.cmd
  io.resp(1).gpa_is_pte   := vstage1_en && r_gpa_is_pte
  io.resp(1).gpa          := {
    val page = Mux(!vstage1_en, Cat(bad_gpa, vpn), r_gpa >> pgIdxBits)
    val offset = Mux(io.resp(1).gpa_is_pte, r_gpa(pgIdxBits-1, 0), io.req(1).bits.vaddr(pgIdxBits-1, 0))
    Cat(page, offset)
  }
  ...
  if (usingVM) {
    ...
    when(io.req(0).fire && vsatp_mode_mismatch) {
